...
# for schema_file in $table_schema_files; 
# do
#   echo "Creating table: $schema_file" 
#   psql -U $POSTGRES_USER -d $DB_NAME -f $SCHEMA_DIR/$schema_file
# done
psql -U $POSTGRES_USER -d $DB_NAME -f $sd/../schema/user.sql
psql -U $POSTGRES_USER -d $DB_NAME -f $sd/../schema/application.sql
psql -U $POSTGRES_USER -d $DB_NAME -f $sd/../schema/vehicle.sql